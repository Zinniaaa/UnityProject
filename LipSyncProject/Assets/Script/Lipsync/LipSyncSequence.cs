...
[System.Serializable]
public class LipSyncSequence : ScriptableObject
{
    public List<LipSync.Frame> entries = new List<LipSync.Frame>();
    public float length;    // in seconds

    public LipSync.Frame GetFrameAtTime(float time)
    {
        LipSync.Frame frame = null;
    ...
    public static LipSyncSequence CreateSequenceFromAudioClip(
        AudioClip clip, bool useOfflineModel = false)
    {
        LipSyncSequence sequence = null;

        if (clip.channels > 2)
        {
            Debug.LogError(clip.name + ": Cannot process phonemes from an audio clip with " +
        ...
        if (LipSync.Initialize(clip.frequency, sSampleSize) != LipSync.Result.Success)
        {
        ...
        LipSync.Result result = useOfflineModel
            ? LipSync.CreateContextWithModelFile(
                ref context,
                LipSync.ContextProviders.Enhanced,
                Path.Combine(Application.dataPath, "Oculus/LipSync/Assets/OfflineModel/ovrlipsync_offline_model.pb"))
            : LipSync.CreateContext(ref context, LipSync.ContextProviders.Enhanced);
        ...
        List<LipSync.Frame> frames = new List<LipSync.Frame>();
        float[] samples = new float[sSampleSize * clip.channels];

        LipSync.Frame dummyFrame = new LipSync.Frame();
        LipSync.ProcessFrame(
            context,
            samples,
            dummyFrame,
        ...
        for (int x = 0; x < totalSamples + frameOffset; x += sSampleSize)
        {
            ...
            LipSync.Frame frame = new LipSync.Frame();
            if (clip.channels == 2)
            {
                // interleaved = stereo data, alternating floats
                LipSync.ProcessFrame(context, samples, frame);
            }
            else
            {
                // mono
                LipSync.ProcessFrame(context, samples, frame, false);
            }
        ...
        Debug.Log(clip.name + " produced " + frames.Count +
            ...
            " fps");
        LipSync.DestroyContext(context);
        LipSync.Shutdown();

        sequence = ScriptableObject.CreateInstance<LipSyncSequence>();
